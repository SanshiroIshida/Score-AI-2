...
async function analyzeScoresheet(imageBase64, mediaType = "image/jpeg") {
  const response = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      max_tokens: 4096, // 選手数が多くてもJSONが切れないよう十分確保
  ...
  // stop_reason が max_tokens の場合は明示的にエラー
  ...
  // コードブロック除去 → JSON部分だけ抽出して parse
  const stripped = text.replace(/```json\n?|```/g, "").trim();

  // { ... } の範囲を抽出（余分なテキストが前後についていても対応）
          ...
          {players.map((p, i) => (
            <tr key={i} style={{ borderBottom: "0.5px solid #eee" }}>
              ...
              <td style={{ padding: "8px 2px" }}>
                <button
                  onClick={() => onEdit(i)}
                  style={{ fontSize: 11, padding: "2px 8px", cursor: "pointer", borderRadius: 4, border: "0.5px solid #ccc", background: "#fff" }}
                >
                  修正
                </button>
              </td>
...
export default function ScoreAI() {
  const [step, setStep] = useState("upload"); // upload | analyzing | result | error
      ...
      {(step === "upload" || step === "error") && (
        <div>
          <div
            ...
            style={{
              border: "2px dashed #B5D4F4", borderRadius: 14, padding: 40,
              textAlign: "center", cursor: "pointer", marginBottom: 16,
              background: imagePreview ? "#f8fbff" : "#fff",
          ...
          <button
            ...
            style={{
              width: "100%", padding: "14px", background: imageBase64 ? "#185FA5" : "#ccc",
              color: "#fff", border: "none", borderRadius: 10, fontSize: 16, fontWeight: 700,
              cursor: imageBase64 ? "pointer" : "not-allowed", transition: "background 0.2s",
            }}
          >
            🤖 AIで解析する
          </button>
          ...
          <div style={{ display: "flex", gap: 10, marginBottom: 16 }}>
            {Object.entries(teamStats).map(([team, s]) => (
              <div key={team} style={{ flex: 1, background: "#f0f6ff", borderRadius: 10, padding: "12px 14px", border: "0.5px solid #B5D4F4" }}>
                ...
                <p style={{ margin: 0, fontSize: 22, fontWeight: 700, color: avgColor(s.hits, s.atBats) }}>
                  {calcAverage(s.hits, s.atBats)}
                </p>
